...
window.PRACTICE_SETS["g2_1"] = [
...

  {
    ...
    img: "images/0032.png",
    options: [
      "ハンマーでたたいて金属（きんぞく）の表面（ひょうめん）に小（ちい）さなくぼみを作（つく）ったり、布（ぬの）や皮（かわ）などに丸（まる）い穴（あな）をあけること",
      "建築用（けんちくよう）の油性（ゆせい）ペン",
      "材料（ざいりょう）の表面（ひょうめん）に長（なが）い直線（ちょくせん）を墨（すみ）付（つ）けすること",
      "粉（こな）チョークで線（せん）を引（ひ）きます"
    ],
    answer: 0,
  ...
  },
  {
    ...
    img: "images/問35.png",
    options: [
      "地面（じめん）に対（たい）して施工面（せこうめん）や物（もの）が、水平（すいへい）になっているかを調（しら）べる道具（どうぐ）である",
      "水準（すいじゅん）測量（そくりょう）の機械（きかい）で、作業（さぎょう）するのに必要（ひつよう）な高さ（たかさ）を出（だ）すのに使（つか）う",
      "小型（こがた）の望遠鏡（ぼうえんきょう）を支（ささ）える視点（してん）を基点（きてん）として上下（じょうげ）方向（ほうこう）、水平（すいへい）方向（ほうこう）の角度（かくど）を測定（そくてい）する機器（きき）である",
...

  {
    ...
    img: "images/問0036.png",
    options: [
      "モルタルの吹付（ふきつけ）は、下部（かぶ）から",
      "吹付面（ふきつけめん）が土砂（どしゃ）の場合（ばあい）、吹付（ふきつけ）の圧力（あつりょく）で土砂（どしゃ）が散乱（さんらん）しないよう注意（ちゅうい）する",
      "吹付（ふきつけ）された岩盤（がんばん）の場合（ばあい）、浮石（うきいし）や泥土（でいど）を先（さき）に除去（じょきょ）する",
...

  {
    ...
    options: [
      "エンジンを切（き）り必（かなら）ずセーフティレバーをロックする",
      "急（いそ）ぎなので、確認（かくにん）せず、乗（の）り降（お）りする",
      "他（ほか）の作業員（さぎょういん）もいるので、動（うご）かしながら乗（の）り降（お）りする",
      "周囲（しゅうい）には立（た）ち入り禁止（きんし）の措置（そち）があるから安全（あんぜん）なのでそのまま乗（の）り降（お）りする"
    ],
    answer: 0,
...

  {
    ...
    options: [
      "１日（いちにち）の作業（さぎょう）終了後（しゅうりょうご）は、安全（あんぜん）な場所（ばしょ）に駐車（ちゅうしゃ）する",
      "バケット等（など）の装置（そうち）を地上（ちじょう）に降（お）ろします",
      "日常（にちじょう）メンテナンスの結果（けっか）は点検表（てんけんひょう）に記録（きろく）します",
      "作業（さぎょう）終了後（しゅうりょうご）すぐにエンジンを切（き）って帰（かえ）る準備（じゅんび）をする"
    ],
    answer: 3,
  ...
  },
{
...

  {
    ...
    options: [
      "溶接棒（ようせつぼう）",
      "溶接ようせつする母材ばざいを接着せっちゃくさせるために使つかわれる金属（きんぞく）棒（ぼう）",
      "熱（ねつ）した鉄（てつ）などをつかむための鉄製（てっせい）の工具（こうぐ）",
      "鉄板（てっぱん）などのケガキ入れに使用（しよう）します"
    ],
    answer: 2,
...

  {
    ...
    options: [
      "トラックアジテータから、生コン（なまこん）を受（う）け取（と）る部分（ぶぶん）",
      "トラックアジテータで運（はこ）ばれてきた生コン（なまこん）を、油圧（ゆあつ）で型枠（かたわく）に送り込（こ）む機械（きかい）",
      "コンクリートの量（りょう）を感知（かんち）する機器（きき）",
      "あらかじめ練（ね）り混（ま）ぜられたコンクリートを固（かた）まらないようにかくはんする装置（そうち）"
    ],
    answer: 0,
    vi: "Hopper (ホッパ) là gì?",
...

  {
    ...
    options: [
      "ワイヤを巻（ま）き上（あ）げたり、送り出（おくりだ）したりすること",
      "重（おも）いものをつり上（あ）げる",
      "クレーンを使（つか）って荷物（にもつ）を吊上（つりあ）げるとき、荷物（にもつ）にワイヤを取（と）り付（つ）けたり、はずしたりする作業（さぎょう）のことです",
      "井戸（いど）を掘（ほ）ること"
    ],
    answer: 2,
  ...
  },
  {
    ...
    options: [
      "舗装（ほそう）道路（どうろ）の中間（ちゅうかん）層（そう）で上層（じょうそう）と下層（かそう）に分（わ）かれている",
      "路盤（ろばん）の上（うえ）の層（そう）で、加熱（かねつ）されたアスファルトをアスファルトフィニッシャーで敷（し）きならす",
      "道路（どうろ）の最下層（さいかそう）の部分（ぶぶん）で厚（あつ）い場合（ばあい）は約（やく）1mの深（ふか）さになる",
...
},
{
  ...
  options: [
    "こびりついた塗料（とりょう）や汚（よご）れを剥（は）がすための道具（どうぐ）",
    "幅（はば）の広（ひろ）い面（めん）の塗装（とそう）を効率（こうりつ）よく塗（ぬ）るための塗装（とそう）用（よう）",
    "パテの混（ま）ぜ合（あ）わせ",
    "下地（したじ）の表面（ひょうめん）の凸凹（でこぼこ）を無（な）くして平（たい）らにする"
  ],
  answer: 0,
...
},
{
  ...
  options: [
    "コンクリートを切断（せつだん）するための電動（でんどう）工具（こうぐ）",
    "細（ほそ）長（なが）いブレードを前後（ぜんご）に動（うご）かすことで材料（ざいりょう）を切断（せつだん）する電動（でんどう）工具（こうぐ）",
    "上部（じょうぶ）に足（あし）をかけて地面（じめん）を掘（ほ）るための道具（どうぐ）",
